-- 1. Fix get_journey_kpis to enforce completed = true for active users
...
BEGIN
  ...
  -- Active: users with at least one COMPLETED module (progress > 0)
  ...
  FROM (
    ...
    WHERE ur.role = 'jovem_aprendiz' AND ump.completed = true
  ) u
  WHERE NOT EXISTS (
...
-- 2. Fix get_journey_phase_distribution to enforce completed = true
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      -- Nao Iniciada = user is currently in this phase, but has NO completed modules for this phase (progress = 0%)
      COUNT(DISTINCT ups.user_id) FILTER (
        ...
        AND NOT EXISTS (
          ...
          WHERE ump.user_id = ups.user_id AND jm.phase_id = ph.id AND ump.completed = true
        )
      )::int AS nao_iniciada,

      -- Em Andamento = user is currently in this phase AND has > 0% progress (at least one completed module)
      COUNT(DISTINCT ups.user_id) FILTER (
        WHERE (
          (COALESCE(ups.status,'pendente') = 'pendente' AND COALESCE(ups.unlocked,false) = true)
          OR ups.status = 'em_andamento'
...
-- 3. Fix admin_get_journey_monitor to require completed = true for in_progress_title
    ...
    SELECT bu.user_id,
      (SELECT ph.title FROM public.user_phase_status ups
        ...
        AND EXISTS (
          SELECT 1 FROM public.user_module_progress ump
          JOIN public.journey_modules jm ON jm.id = ump.module_id
          WHERE ump.user_id = bu.user_id AND jm.phase_id = ph.id AND ump.completed = true
        )
...
-- 4. Fix admin_get_journey_tracking to require completed = true for in_progress_title
