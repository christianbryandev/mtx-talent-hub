-- Garante a versão mais recente e robusta da RPC get_user_journey
...
-- Nova RPC para buscar metadados das fases (usada para badges e resumos)
CREATE OR REPLACE FUNCTION public.get_catalog_phases()
RETURNS jsonb
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN (
    SELECT jsonb_agg(p ORDER BY order_index)
    FROM (
      SELECT id, title, description, order_index, has_quiz, xp_reward
      FROM public.journey_phase_catalog
    ) p
  );
END;
