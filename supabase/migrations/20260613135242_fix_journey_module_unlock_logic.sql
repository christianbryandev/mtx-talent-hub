-- Bug 2 Fix: Corrigir lógica de desbloqueio de módulos na RPC get_user_journey.
-- Problema: A condição usava `jm.order_index = 1` hardcoded para determinar o primeiro módulo.
-- Se o admin deletou módulos e o menor order_index não é 1, nenhum módulo ficava desbloqueado.
-- Fix: Usar MIN(order_index) da fase para identificar o primeiro módulo real.
      ...
      'modules', (
        ...
        FROM (
          SELECT jsonb_build_object(
            ...
            'unlocked', (
              CASE
                -- Primeiro módulo da primeira fase: sempre desbloqueado
                WHEN jm.order_index = (SELECT MIN(m2.order_index) FROM public.journey_modules m2 WHERE m2.phase_id = ph.id)
                     AND ph.order_index = (SELECT MIN(order_index) FROM public.journey_phase_catalog)
                THEN true
                -- Primeiro módulo de qualquer outra fase: desbloqueado se a fase está desbloqueada
                WHEN jm.order_index = (SELECT MIN(m2.order_index) FROM public.journey_modules m2 WHERE m2.phase_id = ph.id)
                THEN COALESCE(ups.unlocked, false)
                -- Demais módulos: desbloqueado se o módulo anterior (por order_index) foi concluído
                ELSE EXISTS (
                  ...
                  WHERE prev.phase_id = ph.id
                    AND prev.order_index = (
                      SELECT MAX(m3.order_index)
                      FROM public.journey_modules m3
                      WHERE m3.phase_id = ph.id AND m3.order_index < jm.order_index
                    )
